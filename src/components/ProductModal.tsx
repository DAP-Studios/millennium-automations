    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[96px_1fr] gap-6 items-start">
            {/* Left: vertical thumbnail list */}
            <div className="flex flex-col gap-2 overflow-y-auto max-h-[420px] md:max-h-[520px] pr-1">
                {images.map((img, i) => (
                  <button
                    key={i}
                    onClick={() => scrollTo(i)}
                    className={`h-20 w-full overflow-hidden rounded-md border flex items-center justify-center bg-white ${i === activeIndex ? 'ring-2 ring-primary/20 border-primary' : 'border-slate-200'}`}
              ...
              <div className="border border-slate-200 rounded-lg overflow-hidden shadow-sm bg-white">
                <div className="embla" ref={emblaRef as any} role="region" aria-roledescription="carousel" aria-label={`${product.title} images`}>
                  <div className="embla__container flex">
                    {images.map((img, i) => (
                      <div key={i} className="embla__slide flex-[0_0_100%] h-64 md:h-[480px] relative overflow-hidden rounded-lg">
